...
class ADAuth {
    ...
    public function lookup_user_bymail($email)
    {
        if($this->ldap_type == "adcontroller") {
            $results = @ldap_search( $this->ldap_conn, 'DC=ad,DC=colorado,DC=edu', "(mail=$email)");
        } else {
            $results = @ldap_search( $this->ldap_conn, 'OU=people,DC=colorado,DC=edu', "(mail={$email})" );
        }
    ...
    public function lookup_group($group)
    {
        $info = array();
        if($this->ldap_type == "adcontroller") {
            $results = @ldap_search( $this->ldap_conn, 'DC=ad,DC=colorado,DC=edu', "(memberof={$group}*)");
        }
        if($results) {
            $info = ldap_get_entries($this->ldap_conn, $results);
        }
